...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function playSound(type) {
    ...
    if (type === 'slide') {
      ...
      notes.forEach((freq, idx) => {
        ...
        noteGain.gain.exponentialRampToValueAtTime(0.01, now + idx * 0.09 + 0.25);
  ...
  function updateTilePositions(animate = true) {
    ...
    tiles.forEach((tileObj, currentIndex) => {
      ...
      tileObj.element.style.transition = animate ? 'transform 0.18s cubic-bezier(0.2, 0, 0, 1)' : 'none';
  ...
  function shufflePuzzle() {
    ...
    shuffleInterval = setInterval(() => {
      const validNeighbors = getValidNeighbors(emptyTileIndex).filter(idx => idx !== lastMovedIndex);
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowUp' && emptyRow < gridSize - 1) targetIndex = emptyTileIndex + gridSize;
    if (e.key === 'ArrowDown' && emptyRow > 0) targetIndex = emptyTileIndex - gridSize;
    if (e.key === 'ArrowLeft' && emptyCol < gridSize - 1) targetIndex = emptyTileIndex + 1;
    if (e.key === 'ArrowRight' && emptyCol > 0) targetIndex = emptyTileIndex - 1;
  ...
  function saveScore(grid, movesCount, timeInSeconds) {
    const scores = JSON.parse(localStorage.getItem('taquin_leaderboard') || '{}');
  ...
  function updateLeaderboardUI(tabGridSize = 3) {
    ...
    if (list.length === 0) {
      leaderboardList.innerHTML = '<li><span>Aucun score enregistré</span></li>';
      return;
    }
  ...
  uiThemeButtons.forEach(button => {
    button.addEventListener('click', () => applyUiTheme(button.dataset.uiTheme));
  });
  ...
  document.getElementById('btn-random-game').addEventListener('click', startRandomGame);
  document.getElementById('btn-back-to-menu').addEventListener('click', showMenu);
  document.getElementById('btn-victory-menu').addEventListener('click', showMenu);
  document.getElementById('btn-shuffle').addEventListener('click', shufflePuzzle);
  document.getElementById('btn-restart').addEventListener('click', shufflePuzzle);
  ...
  if (savedUiTheme === 'light' || savedUiTheme === 'dark') applyUiTheme(savedUiTheme);
