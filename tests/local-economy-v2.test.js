...
assert(ArcadeLocalStore.getActiveProfile().balanceUnits === 600, "Une victoire doit rendre la mise et verser celle de l’adversaire");
ArcadeLocalStore.finishSession(won.id, "won", { duplicate: true });
assert(ArcadeLocalStore.getActiveProfile().balanceUnits === 600, "Un résultat dupliqué ne doit pas repayer");
...
assert(ArcadeLocalStore.getActiveProfile().balanceUnits === 500, "Un abandon après démarrage doit perdre la mise");
...
assert(ArcadeLocalStore.getActiveProfile().balanceUnits === 400, "Une défaite doit perdre la mise");
...
assert(ArcadeLocalStore.getActiveProfile().balanceUnits === 400, "Un jeu d’entraînement ne doit jamais débiter");
...
assert(ArcadeLocalStore.getActiveProfile().balanceUnits === 400, "Un mode sans fin doit pouvoir devenir gratuit au démarrage");
...
assert(ArcadeLocalStore.getActiveProfile().balanceUnits === 300, "La récupération ne doit pas rembourser une session commencée");
...
assert(alice.balanceUnits === 400, "Les ajustements administrateur doivent être appliqués");
